<template x-if="showRestore">
	<div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-4 py-6 sm:p-0">
		...
		<div class="relative w-full sm:max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-xl ring-1 ring-gray-200 dark:ring-gray-700 p-6 flex flex-col gap-6">
			...
			<form class="space-y-5 text-sm" @submit.prevent="alert('Pulihkan (dummy)'); closeAll()">
				<div class="bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 rounded-lg p-4 text-xs text-amber-700 dark:text-amber-300 flex items-start gap-3">
					...
					<div>Memulihkan arsip akan mengubah status kembali ke <span class="font-semibold">published</span> (atau sesuai status terakhir sebelum arsip) dan kembali muncul di daftar aktif.</div>
				</div>
				<div>
					<label class="block text-[11px] font-medium mb-1 text-gray-600 dark:text-gray-300">Catatan Pemulihan (opsional)</label>
					<textarea rows="4" class="w-full rounded-lg bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-slate-400 dark:focus:ring-slate-500 text-gray-700 dark:text-gray-100" placeholder="Alasan memulihkan arsip..."></textarea>
				</div>
